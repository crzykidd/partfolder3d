...
| [release-prep-and-cut](https://gitea.crzynet.com/crzynet/homelab-configs/src/branch/main/standards/release-prep-and-cut/README.md) | 1.1.0 | 2026-06-27 | `/release-prep` + `/release-cut` templates copied to `.claude/commands/`; `CLAUDE.md` snippet pasted. **Filled (Phase 10b):** all placeholders replaced — version file `backend/app/version.py` (`__version__ = "0.1.0"`), `frontend/package.json` version sync, image `ghcr.io/crzykidd/partfolder3d`, publish workflow `Build and publish Docker images`, local checks matching CI, archive dir `docs/`. Ready to run `/release-prep <version>`. Composes with `code-checkin-and-pr`. |
