...
|---|---|---|---|
| [code-checkin-and-pr](https://gitea.crzynet.com/crzynet/homelab-configs/src/branch/main/standards/code-checkin-and-pr/README.md) | 1.2.0 | 2026-06-27 | Branch strategy (`dev`/protected `main`) + commit conventions + `CLAUDE.md` snippet **in effect now**. CI checks (lint, config, migration, compose, image build, tests, SAST), image-publish matrix, and registry retention are **phased** — wired during scaffolding as the app, Dockerfiles, and CI come online (repo with no tests is exempt until it adds one). **Deviation:** auto-commit on `dev` (no per-step y/n) per the project operating model; `main` stays PR-only. No `Co-authored-by:` trailers. |
