...
| Import sessions / bulk import | `import_sessions/` (`sessions.py`, `helpers.py`, `commit.py`, `__init__.py`) | `import_session.py` (`ImportSessionFile.selected` — per-file commit toggle, Manyfold Part 2) | `tasks/import_session.py` (`_maybe_manyfold_import` — Manyfold primary path, tried BEFORE `scrape_url`/the fallback chain when the URL's domain matches an enabled `ManyfoldInstance`) | `storage/scraper.py`, `storage/archive.py`, `storage/manyfold_client.py` | `import.ts` | `ImportsPage.tsx`, `ImportWizardPage.tsx`, `pages/import-wizard/*` | `test_phase5_import.py`, `test_bulk_import.py`, `test_import_management.py`, `test_url_wizard_attach.py`, `test_manyfold_import.py`; FE `import-wizard.test.ts`, `bulk-import.test.ts`, `import-wizard-steps.test.tsx` |
...
| Manyfold connector (Parts 1–2 of 3 landed — config + admin API + worker import path; frontend in Part 3) | `manyfold.py` | `manyfold.py` (`ManyfoldInstance`) | `tasks/import_session.py` (`_maybe_manyfold_import`) | `storage/manyfold_client.py` (`fetch_token`/`get_access_token`/`get_cached_access_token`, `parse_model_id`, `fetch_model`, `resolve_creator`, `download_file` — OAuth2 client_credentials + model/file JSON-LD API) | — (Part 3) | — (Part 3) | `test_manyfold_admin.py`, `test_manyfold_import.py` |
